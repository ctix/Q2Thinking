...
student_model_name = "distilgpt2" # Changed from google/gemma-2b-it due to gated access
...
# Handle potential missing pad_token for models like gpt2 that don't have it by default.
if tokenizer.pad_token_id is None:
    if tokenizer.eos_token_id is not None:
        print("Tokenizer missing pad_token_id, using eos_token_id as pad_token_id.")
        tokenizer.pad_token_id = tokenizer.eos_token_id
    else:
        # Add a new pad token if eos is also missing
        print("Tokenizer missing pad_token_id and eos_token_id. Adding a new pad token '[PAD]'.")
        tokenizer.add_special_tokens({'pad_token': '[PAD]'})
        # Important: If a new token is added, student_model embeddings might need resizing
        # This should be done after loading the student_model.
...
# If a new pad token was added to the tokenizer, resize student model token embeddings
if tokenizer.pad_token == '[PAD]' and tokenizer.pad_token_id >= student_model.config.vocab_size:
     print(f"Resizing student model token embeddings to include newly added pad token. Vocab size: {len(tokenizer)}")
